...
void zone_build_subzones(zone_t *zone, vertices_view_t vertices, edges_view_t edges, arena_t *arena, arena_t scratch) {
    ...
    subzone_t *subzone = subzones_get(&zone->subzones, i);
...
aabb2f_t zone_get_aabb(const zone_t *zone, vertices_view_t vertices, edges_view_t edges) {
    ...
    aabb2f_t aabb = edge_get_aabb(edges_view_get(edges, element_ids_get(perimeter_edge_ids, 0)), vertices);
    for (uint32_t i = 1; i < perimeter_edge_ids->num; ++i) {
        aabb = aabb2f_union(aabb, edge_get_aabb(edges_view_get(edges, element_ids_get(perimeter_edge_ids, i)), vertices));
    }
    return aabb;
}
