...
#define array_init_copy_reserve(name, arena, src, size) (array_init_reserve(name, arena, max_uint32((size), src##_num)), memcpy(name, src, src##_num * sizeof(*name)), name##_num = src##_num)
...
#define array_push(name, arena) (array_grow(name, arena, name##_num + 1, (name##_num + 1) * 2), name##_num++)
